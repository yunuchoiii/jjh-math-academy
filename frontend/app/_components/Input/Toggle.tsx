...
const Toggle = ({ checked, onChange, label }: ToggleProps) => {
  return <StyledToggle 
    className="flex items-center justify-between gap-4 mb-4 cursor-pointer px-1" 
    onClick={() => onChange(!checked)}
  >
    <span className="text-sm">{label}</span>
    <div 
      className={`relative w-[60px] h-[30px] rounded-full shadow-2 ${checked ? "bg-green-1" : "bg-[#C9C9C9]"} transition-all duration-300`}
    >
