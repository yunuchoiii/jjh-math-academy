  ...
  return <>
    <section className='hidden md:block'>
      {curriculumList.map((item, index) =>  (
        ...
        >
          <header className={`xl:w-[218px] xl:h-[218px] md:w-48 md:h-48 rounded-full bg-gradient-to-b absolute xl:bottom-9 md:bottom-7 shadow-xl ${index == 0 ? 'from-yellow-1 to-yellow-3' : 'from-green-1 to-green-2'}`}>
            <div className='absolute bottom-0 w-full flex justify-center rounded-bl-full rounded-br-full overflow-hidden'>
              <Image 
                src={item.imgPath} 
                alt='curriculum' 
                className={`${index == 0 ? 'xl:w-36 md:w-32' : 'w-full'}`} 
                loading='lazy' 
                width={144} 
                height={144}
              />
          ...
          <div className='xl:pt-10 md:pt-7 xl:pl-72 md:pl-60 pr-10 font-bold'>
            <h2 className='xl:text-xl md:text-lg text-[#555] xl:mb-2.5 md:mb-1.5'>
              {item.desc}
            </h2>
            <h1 className='xl:text-4xl md:text-3xl' style={{color: item.color}}>
              {item.title}
            </h1>
          ...
          <div className='absolute bottom-6 right-8 flex items-center'>
            ...
            <div className='xl:w-7 xl:h-7 md:w-6 md:h-6 rounded-3xl flex items-center justify-center ml-3' style={{backgroundColor: item.iconColor}}>
              <Image 
                ...
                alt='plus'
                className='xl:w-4 xl:h-4 md:w-3 md:h-3 invert' 
                width={16} 
                height={16} 
                loading='lazy'
              />
          ...
          <div className={`w-40 h-40 relative rounded-full bg-gradient-to-b shadow-xl ${index == 0 ? 'from-yellow-1 to-yellow-3' : 'from-green-1 to-green-2'}`}>
            <div className='absolute bottom-0 w-full flex justify-center rounded-bl-full rounded-br-full overflow-hidden'>
              <Image 
                src={item.imgPath} 
                alt='curriculum' 
                className={`${index == 0 ? 'xl:w-36 md:w-32 w-[104px]' : 'w-full'}`} 
                loading='lazy' 
                width={144} 
                height={144}
              />
